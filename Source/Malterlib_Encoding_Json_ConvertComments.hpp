...
{
	// JSONC trivia includes whitespace and comment delimiters. YAML metadata stores newline-terminated
	// '#' comment lines and bare blank lines; inline comments have no newline terminator.
	...
	inline umint fg_JsonTriviaFindSameLineEnd(NStr::CStr const &_Trivia)
	{
		...
		while (pParse < pEnd)
		{
			if (pParse[0] == '/' && pParse + 1 < pEnd && pParse[1] == '*')
			{
				auto const *pComment = pParse;
				pParse += 2;
				while (pParse < pEnd && !(pParse[0] == '*' && pParse + 1 < pEnd && pParse[1] == '/'))
				{
					// Multiline block comments must remain together in the trailing YAML comment lines.
					if (*pParse == '\n' || *pParse == '\r')
						return pComment - pStart;
					++pParse;
				}
				pParse = pParse < pEnd ? pParse + 2 : pEnd;
				continue;
			}
	...
	// Records follow the destination traversal order so comments survive moves and object-key sorting.
	// Empty strings represent absent comment slots.
			...
			auto fRecordEntries = [&](auto _iSrc)
				{
					while (_iSrc)
					{
						...
						// YAML emits one leading comment block per entry, so merge the key and value trivia.
			...
			auto fRecordEntries = [&](auto _iSrc)
				{
					while (_iSrc)
					{
						...
						// Match the YAML generator's key-first selection to avoid duplicating shared leading comments.
