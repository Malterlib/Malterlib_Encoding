		...
		void f_DoTestsImpl(NStr::CStr const &_Description)
		{
			...
			DMibTestSuite(_Description)
			{
				...
				DMibTestCategory("JsonCommentConversion")
				{
					...
					auto MultiLineJsonC = CJsonWithComments::fs_FromStringJsonC
						(
							"{\n"
							"	\"a\": 1, /* first\n"
							"	second */\n"
					...
					auto MultiLineYaml = CJsonWithMeta::fs_FromCompatible(MultiLineJsonC);
					DMibExpect(MultiLineYaml["a"].f_Yaml().f_LineComment(), ==, "");
					DMibExpect(MultiLineYaml["a"].f_Yaml().f_TrailingComment(), ==, "# first\n# second\n");
					DMibExpect(MultiLineYaml["b"].f_Yaml().f_LineComment(), ==, "");
					DMibExpect(MultiLineYaml["b"].f_Yaml().f_TrailingComment(), ==, "# tail first\n# tail second\n");

					auto MultiLineYamlText = MultiLineYaml.f_ToStringYaml();
					DMibExpect
						(
							MultiLineYamlText
							, ==
					...
					;
					DMibExpect(CJsonWithMeta::fs_FromStringYaml(MultiLineYamlText).f_ToStringYaml(), ==, MultiLineYamlText);

					auto MultiLineRoundTripped = CJsonWithComments::fs_FromCompatible(fg_Move(MultiLineYaml));
					auto MultiLineJsonCText = MultiLineRoundTripped.f_ToStringJsonC();
					DMibExpect
						(
							MultiLineJsonCText
							, ==
					...
					;
					DMibExpect(CJsonWithComments::fs_FromStringJsonC(MultiLineJsonCText).f_ToStringJsonC(), ==, MultiLineJsonCText);
				};
			};
		}
